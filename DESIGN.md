...
| 书架来源 | 扫描本地目录 | 远端快照↔（无 add API，delete↑） | 远端快照↔ + addToShelf/delete |
...
关书时 progress/notes/stats 一律 `dirty_only` 只推不拉。部分远端服务存在收敛延迟，立即回拉会用旧值覆盖刚上传的新值。

### 8.3 同步契约（本地优先）

所有域统一：**本地优先 push，再 pull 云端**。特殊时机：

| 域 | Push | Pull |
|---|---|---|
| 书架 / 统计 | 全量同步：先推脏删加/脏行，再拉远端对账 | 全量同步时 pull；`dirty_only` / 网络恢复只推不拉 |
| 进度 | 关书 / 网络恢复 / 面板同步：只推脏 | **仅开书**（有网）`Progress.pull`：拉云端对比，冲突交给用户选 |
| 笔记 | 有网即推（划线改动 / 关书 / 网络恢复） | **仅开书** `Note.pull` |

`book/sync.lua` 按书架 → 进度 → 笔记 → 统计编排。进度/笔记在编排层**永远** `dirty_only`（禁止静默 `upsertRemote`）。网络恢复经 `Sync.retryDirtyAsync`：有 books 脏行时跑书架域且 `dirty_only`（只推删/加，不做全量 shelf pull）。任一远端失败不能删除本地待同步状态。

书架：本地删标 `deleted` → 同步时推云端真删；本地加架脏行推 `addToShelf`；全量时再 reconcile 远端快照。moon 无加书架 API；local 扫盘。

### 8.4 章节阅读

`source/chapter.lua` 提供按章来源的公共打开、预取和切章逻辑。起始章优先级为：

1. 本地待同步进度（`pending_progress`）；
2. 远端进度。
...
- 列表封面可以叠加黑底白字的百分比角标；角标只在进度大于 0 时显示。图书馆另在右下角显示“新/读/未”单字折角，优先级为“新书 > 已读 > 未读”。
...
- 图书馆不提供独立筛选器；“视图”入口在书架、分类、系列和阅读状态四种全局记忆的视图间切换。
- 阅读状态与进度：手动已读会把进度抬到 100%（脏写待同步进度，云端由进度同步收敛已读标签）；手动未读不回退进度，且阻止 99% 自动规则再次覆盖；100% 完成始终标记已读；新书在首次打开阅读后清除。
- 分类、系列和阅读状态视图先显示分组及书籍数，点击后进入对应书架；空分类归入“未分类”，空系列归入“无系列”，阅读状态固定为“新书/已读/未读”三组。
- 书城复用图书馆的封面网格和分页，不复制一套列表布局；书城只额外提供搜索。
- 网格单元为“封面 + 单行书名”；整格可点，长按可标记已读/未读或删除（在线源删除经 `deleteBookAsync` 同步云端；手动已读抬进度到 100% 后经进度同步收敛云端），封面可带进度角标和状态绑带。
