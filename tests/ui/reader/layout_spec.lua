...
Assert.eq(Layout.get("initial").summary, "首段不缩进 · 标题左起")
Assert.is_true(Layout.get("essay").css:find("text%-indent: 0") ~= nil)
-- 起首不再做首字下沉，避免 CREngine 因 display/float 变化把 DOM 标记为 stale。
Assert.is_nil(Layout.get("initial").css:find("initial%-letter"))
...
Assert.is_nil(ui.styletweak.book_style_tweak:find("initial%-letter"))
