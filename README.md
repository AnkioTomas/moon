...
保存后可使用 **测试连接** 验证配置。设置页还可控制自动同步进度、启动时打开桌面、主页顶部内容和界面字号。
...
2. 在图书馆顶部使用搜索、筛选或清除筛选；左右滑动可切换页面。
3. 点按一本书打开详情页，然后选择 **开始阅读**。书籍不存在于本地时会自动下载。
...
插件设置保存于 KOReader 的 `book_plugin_v2` 设置项。书籍下载到配置的本地目录，封面保存在其中的 `.covers` 子目录；本地文件与远端文件名的映射保存于 `book_plugin_filemap_v2`，用于进度同步。
