...
月读是一个面向 [KOReader](https://koreader.rocks/) 的本地优先书库插件。
...
在 **设置 → 数据源 → 月读服务** 中填写服务地址和令牌并保存。
该源需要兼容的月读服务端；服务端项目见 [AnkioTomas/book](https://github.com/AnkioTomas/book)。
...
3. 阅读时点按屏幕中部打开月读面板；可从面板进入目录、调整排版或返回书库。
