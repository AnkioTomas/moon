...
local function pickLockScreen(desktop, current)
    local items = LockScreen.options()
    Popup.list{
        title = _("锁屏显示"),
        items = items,
        current = current,
        ...
        on_select = function(mode)
            ...
            if LockScreen.canRefreshOffline(mode) then
                refresh()
            else
...
local function lockscreenRows(desktop)
    ...
    if mode == "quote" then
        rows[#rows + 1] = function(iw)
            ...
            local mode = LockScreen.quoteMode()
            return SettingRow.build(iw, {
                kind = "nav",
                icon = "format_quote",
                title = _("语句来源"),
                ...
                callback = function()
                    Popup.list{
                        ...
                        current = mode, choice_icons = true, centered = true,
                        on_select = function(value)
        ...
        rows[#rows + 1] = function(iw)
            ...
            return SettingRow.build(iw, {
                ...
                callback = function()
                    local items = {}
                    for _, value in ipairs({ "top-left", "top-center", "top-right", "center-left", "center-center", "center-right", "bottom-left", "bottom-center", "bottom-right" }) do
                        items[#items + 1] = { text = positions[value], value = value }
                    end
                    Popup.list{ title = _("显示位置"), items = items, current = position, choice_icons = true, centered = true, on_select = function(value)
        ...
        rows[#rows + 1] = function(iw)
            ...
            return SettingRow.build(iw, {
                kind = "toggle", icon = "aspect_ratio", title = _("宽屏显示"), status = wide and _("开") or _("关"), status_on = wide,
                callback = function()
