...
*::first-letter { initial-letter: none !important; font-size: inherit !important; }
]]
...
local MANAGED_TWEAK_SET = {}
for _, id in ipairs(MANAGED_TWEAK_IDS) do
    MANAGED_TWEAK_SET[id] = true
end
...
local function presetMatchesConfig(preset, config)
    local h = config.h_page_margins
    return tonumber(config.font_size) == preset.font_size
        and tonumber(config.line_spacing) == preset.line_spacing
        and tonumber(config.font_base_weight) == preset.font_base_weight
        and type(h) == "table"
...
local function applyCss(ui, preset)
    ...
    local managed = MANAGED_TWEAK_SET
...
function Layout.currentPreset()
    local id = Settings.get().book_reader_layout_id
...
function Layout.matchId(ui)
    ...
    for _, preset in ipairs(PRESETS) do
        if presetMatchesConfig(preset, config) then
            return preset.id
        end
    end
    return "off"
...
function Layout.disable(ui)
    ...
    require("ui/widget/notification").notify(_("已关闭阅读风格预设"))
    return true
end
...
function Layout.apply(ui, preset)
    ...
    require("ui/widget/notification").notify(T(_("已应用「%1」并设为默认"), preset.title))
    return true
end
