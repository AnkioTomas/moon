...
Book (moon) HTTP 客户端（Bearer Token）
  仅由 source.moon 适配器使用，UI 不得直接 require。
...
function Api:_request(method, path, query, body_tbl, sink_file, binary, as_json)
    ...
    if not data then
        ...
        return nil, T(_("响应不是 JSON (HTTP %1) %2"), tostring(code), previewBody(raw))
    end
...
function Api.hitokoto()
    ...
    if not jok or type(data) ~= "table" then
        return nil, _("一言响应不是 JSON")
    end
    if code ~= 200 or (data.code and data.code ~= 200) then
        return nil, data.msg or T(_("一言 HTTP %1"), tostring(code))
    end
...
function Api:downloadCover(filename, dest_path)
    ...
    if not renamed then
        ...
        if not (rf and wf) then
            ...
            pcall(os.remove, tmp)
            return nil, rename_err or _("封面改名失败")
        end
