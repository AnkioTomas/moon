...
---@param headers table
---@param signed table
local function mergeSigned(headers, signed)
...
local function registerKeyAsync(settings, cb)
    ...
    local khronos = math.floor(ts_ms / 1000)
    local query = Crypto.build_common_query(device, ts_ms)
    local qs = Crypto.encode_query(query)
    local headers = Crypto.build_unsigned_headers(device, ts_ms)
    headers["Content-Type"] = "application/json"
    mergeSigned(headers, Sign.sign_headers(qs, device.device_id, {
        khronos = khronos,
    ...
    local url = "https://" .. HOST .. Crypto.REGISTERKEY_PATH .. "?" .. qs
    return Request.request({
        ...
        if err or not res then
            cb(nil, err or "registerkey 请求失败")
            return
        end
        local code = tonumber(res.code)
        if not Request.ok(code) then
            cb(nil, "registerkey HTTP " .. tostring(code))
            return
        end
        local body_text = res.body or ""
        if body_text == "" then
            cb(nil, "registerkey 空响应（设备身份可能被风控）")
            return
        end
        local data, decode_err = decodeJson(body_text)
        if not data then
            cb(nil, decode_err or "registerkey 响应无效")
            return
        end
        if tonumber(data.code) ~= 0 or type(data.data) ~= "table" or not data.data.key then
...
local function batchFullAsync(settings, book_id, item_id, v1_key, cb)
    ...
    local ts_ms = os.time() * 1000 + (math.random(0, 999))
    local khronos = math.floor(ts_ms / 1000)
    local query = Crypto.build_batch_full_query(device, book_id, { item_id }, ts_ms)
    local qs = Crypto.encode_query(query)
    local headers = Crypto.build_unsigned_headers(device, ts_ms)
    mergeSigned(headers, Sign.sign_headers(qs, device.device_id, {
        khronos = khronos,
    ...
    headers["X-Ss-Req-Ticket"] = tostring(ts_ms)
    local url = "https://" .. HOST .. Crypto.BATCH_FULL_PATH .. "?" .. qs
    return Request.request({
        url = url,
        method = "GET",
        headers = headers,
        ...
        if err or not res then
            cb(nil, err or "batch_full 请求失败")
            return
        end
        local code = tonumber(res.code)
        if not Request.ok(code) then
            cb(nil, "batch_full HTTP " .. tostring(code))
            return
        end
        local body_text = res.body or ""
        if body_text == "" then
            cb(nil, "batch_full 空响应（设备身份可能被风控）")
            return
        end
        local data, decode_err = decodeJson(body_text)
        if not data then
            cb(nil, decode_err or "batch_full 响应无效")
            return
        end
        if tonumber(data.code) ~= 0 or type(data.data) ~= "table" then
            cb(nil, "batch_full 失败: " .. tostring(data.message or data.msg or data.code))
            return
        end
